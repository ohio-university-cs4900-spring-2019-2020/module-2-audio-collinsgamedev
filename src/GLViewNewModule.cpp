...
void GLViewNewModule::onCreate()
{
   ...
   this->manager = sound_manager::init();
   this->manager->play_2d_sound("../mm/sounds/Undertale - Megalovania.wav", true, false, true);
   this->manager->get_2d_sound().at(0)->setVolume(0.25f);
...
void GLViewNewModule::updateWorld()
{
   ...
   if (!this->manager->get_3d_sound().empty())
   {
	   this->manager->get_sound_eng()->setListenerPosition(
		   this->manager->position(this->cam->getPosition()),
		   this->manager->position(this->cam->getLookDirection()), 
		   irrklang::vec3df(0, 0, 0),
		   this->manager->position(this->cam->getNormalDirection()));
   }
}
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if( key.keysym.sym == SDLK_j )
   {
	   if (!this->manager->get_3d_sound().empty() && !this->manager->get_3d_sound().at(0)->isFinished())
	   {
		   this->manager->play_3d_sound("../mm/sounds/Jebaited Song.wav", Vector(40, 10, 40), true, false, true);
		   this->manager->get_3d_sound().back()->setVolume(0.5f);
		   this->manager->get_3d_sound().back()->setMinDistance(7.0f);
